//! ThorLabs MDT693/694/695 piezo motor IOC.
//!
//! Assembles: a serial octet port (`drvAsynSerialPortConfigure`), the MDT695
//! iocsh commands (`MDT695Setup`, `MDT695Config`), the motor record type, and
...
use motor_thorlabs::ioc::{mdt695_config_command, mdt695_setup_command};
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: thorlabs-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("THORLABS_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // MDT695 iocsh commands + motor device support.
    let holder = MotorHolder::new();
    app = app.register_startup_command(mdt695_setup_command());
    app = app.register_startup_command(mdt695_config_command(&holder));
