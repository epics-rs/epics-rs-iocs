//! Micos SMC corvus motor IOC.
//!
//! Assembles: an asyn octet port (`drvAsynIPPortConfigure` or
//! `drvAsynSerialPortConfigure`), the corvus iocsh command
//! (`SMCcorvusCreateController`), the motor record type, and the CA + PVA (QSRV)
//! server.
//!
//! Usage:
...
use motor_micos::ioc::corvus_create_controller_command;
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: micos-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("MICOS_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // Corvus iocsh command + motor device support.
