//! Animatics SmartMotor servo motor IOC.
//!
//! Assembles: a serial octet port (`drvAsynSerialPortConfigure`), the
//! SmartMotor iocsh commands (`SmartMotorSetup`, `SmartMotorConfig`), the motor
...
use motor_smartmotor::ioc::{smartmotor_config_command, smartmotor_setup_command};
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: smartmotor-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("SMARTMOTOR_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // SmartMotor iocsh commands + motor device support.
    let holder = MotorHolder::new();
    app = app.register_startup_command(smartmotor_setup_command());
    app = app.register_startup_command(smartmotor_config_command(&holder));
