//! Mclennan PM304 / PM600 stepper motor IOC.
//!
//! Assembles: a serial octet port (`drvAsynSerialPortConfigure`), the PM304
//! iocsh commands (`PM304Setup`, `PM304Config`), the motor record type, and the
//! CA + PVA (QSRV) server.
//!
//! Usage:
...
use motor_mclennan::ioc::{pm304_config_command, pm304_setup_command};
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: mclennan-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("MCLENNAN_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // PM304 iocsh commands + motor device support.
    let holder = MotorHolder::new();
    app = app.register_startup_command(pm304_setup_command());
    app = app.register_startup_command(pm304_config_command(&holder));
