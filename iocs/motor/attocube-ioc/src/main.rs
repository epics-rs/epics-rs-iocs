//! attocube ANC150 piezo stepper motor IOC.
//!
//! Assembles: a serial octet port (`drvAsynSerialPortConfigure`), the ANC150
//! iocsh commands (`ANC150AsynSetup`, `ANC150AsynConfig`), the motor record
...
use motor_attocube::ioc::{anc150_config_command, anc150_setup_command};
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: attocube-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("ATTOCUBE_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // ANC150 iocsh commands + motor device support.
    let holder = MotorHolder::new();
    app = app.register_startup_command(anc150_setup_command());
    app = app.register_startup_command(anc150_config_command(&holder));
