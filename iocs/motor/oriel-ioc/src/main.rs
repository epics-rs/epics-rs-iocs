//! Oriel Encoder Mike 18011 (EMC18011) motor IOC.
//!
//! Assembles: a serial octet port (`drvAsynSerialPortConfigure`), the EMC18011
//! iocsh commands (`EMC18011Setup`, `EMC18011Config`), the motor record type,
...
use motor_oriel::ioc::{emc18011_config_command, emc18011_setup_command};
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: oriel-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("ORIEL_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // EMC18011 iocsh commands + motor device support.
    let holder = MotorHolder::new();
    app = app.register_startup_command(emc18011_setup_command());
    app = app.register_startup_command(emc18011_config_command(&holder));
