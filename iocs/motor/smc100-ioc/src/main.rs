//! Newport SMC100 motor IOC.
//!
//! Assembles: a serial octet port (`drvAsynSerialPortConfigure`), the Newport
//! SMC100 controller (`SMC100CreateController`), the motor record type, and the
//! CA + PVA (QSRV) server.
//!
//! Usage:
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: smc100-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("SMC100_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // which creates the serial octet port the SMC100 driver connects to.
    ...
    // SMC100CreateController command + motor device support.
    let holder = NewportHolder::new();
    app = app.register_startup_command(holder.smc100_create_controller_command());
