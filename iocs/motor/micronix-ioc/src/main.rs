//! Micronix MMC-100/200 motor IOC.
//!
//! Assembles: a serial octet port (`drvAsynSerialPortConfigure`), the Micronix
//! MMC-100/200 controller (`MMC200CreateController`), the motor record type,
...
use motor_micronix::ioc::mmc200_create_controller_command;
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: micronix-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("MICRONIX_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // MMC200CreateController command + motor device support.
    let holder = MotorHolder::new();
    app = app.register_startup_command(mmc200_create_controller_command(&holder));
