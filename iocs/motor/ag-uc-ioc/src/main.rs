//! Newport Agilis AG-UC motor IOC.
//!
//! Assembles: a serial octet port (`drvAsynSerialPortConfigure`), an Agilis
//! controller (`AG_UCCreateController`) with its axes (`AG_UCCreateAxis`), the
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: ag-uc-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("AG_UC_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // Agilis controller + per-axis create commands + motor device support.
    let holder = NewportHolder::new();
    app = app.register_startup_command(holder.ag_uc_create_controller_command());
    app = app.register_startup_command(holder.ag_uc_create_axis_command());
