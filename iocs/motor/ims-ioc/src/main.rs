//! IMS MDrivePlus motor IOC.
//!
//! Assembles: an asyn octet port (`drvAsynIPPortConfigure` or
//! `drvAsynSerialPortConfigure`), the IMS iocsh command
//! (`ImsMDrivePlusCreateController`), the motor record type, and the CA + PVA
//! (QSRV) server.
//!
//! Usage:
...
use motor_ims::ioc::ims_create_controller_command;
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: ims-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("IMS_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // IMS config command creates the controller + its single axis on one motor
    // device support (DTYP = motorPort).
    let holder = MotorHolder::new();
    app = app.register_startup_command(ims_create_controller_command(&holder));
