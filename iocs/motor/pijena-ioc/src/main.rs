//! piezosystem jena E-516 (PIJEDS) closed-loop piezo motor IOC.
//!
//! Assembles: a serial octet port (`drvAsynSerialPortConfigure`), the PIJEDS
//! iocsh commands (`PIJEDSSetup`, `PIJEDSConfig`), the motor record type, and
...
use motor_pijena::ioc::{pijeds_config_command, pijeds_setup_command};
...
async fn main() -> CaResult<()> {
    ...
    let script = if args.len() > 1 && !args[1].starts_with('-') {
        ...
        eprintln!("Usage: pijena-ioc <st.cmd>");
        std::process::exit(1);
    };

    epics_rs::base::runtime::env::set_default("PIJENA_IOC", env!("CARGO_MANIFEST_DIR"));
    ...
    // PIJEDS iocsh commands + motor device support.
    let holder = MotorHolder::new();
    app = app.register_startup_command(pijeds_setup_command());
    app = app.register_startup_command(pijeds_config_command(&holder));
