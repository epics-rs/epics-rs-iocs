...
dbLoadRecords("$(ADCORE)/db/NDStdArrays.template", "P=$(PREFIX),R=image2:,PORT=IMAGE2,NDARRAY_PORT=$(DEPTH_PORT),TYPE=Int16,FTVL=SHORT,NELEMENTS=$(NELEMENTS_DEPTH)")
...
dbLoadRecords("$(ADCORE)/db/NDPva.template", "P=$(PREFIX),R=depthPva1:,PORT=PVA2,NDARRAY_PORT=$(DEPTH_PORT),ENABLED=1")

# ===== ROI + ROIStat for region analysis =====
NDROIConfigure("ROI1_D", $(QSIZE), 0, "$(DEPTH_PORT)", 0)
dbLoadRecords("$(ADCORE)/db/NDROI.template", "P=$(PREFIX),R=depthROI1:,PORT=ROI1_D,NDARRAY_PORT=$(DEPTH_PORT)")

NDROIStatConfigure("ROIStat1_D", $(QSIZE), 0, "$(DEPTH_PORT)", 0, 8)
dbLoadRecords("$(ADCORE)/db/NDROIStat.template", "P=$(PREFIX),R=depthROIStat1:,PORT=ROIStat1_D,NDARRAY_PORT=$(DEPTH_PORT),NCHANS=$(NCHANS)")
...
dbLoadRecords("$(ADCORE)/db/NDStats.template", "P=$(PREFIX),R=depthStats1:,PORT=STATS1_D,NDARRAY_PORT=$(DEPTH_PORT),NCHANS=$(NCHANS),XSIZE=$(XSIZE),YSIZE=$(YSIZE),HIST_SIZE=$(HIST_SIZE)")
NDTimeSeriesConfigure("STATS1_D_TS", $(QSIZE), 0, "STATS1_D", 0)
dbLoadRecords("$(ADCORE)/db/NDTimeSeries.template", "P=$(PREFIX),R=depthStats1:TS:,PORT=STATS1_D_TS,ADDR=0,TIMEOUT=1,NDARRAY_PORT=STATS1_D,NDARRAY_ADDR=0,NCHANS=$(NCHANS),ENABLED=1")

# ===== File savers: TIFF (16-bit) and HDF5 =====
NDFileTIFFConfigure("FileTIFF1_D", $(QSIZE), 0, "$(DEPTH_PORT)", 0)
dbLoadRecords("$(ADCORE)/db/NDFileTIFF.template", "P=$(PREFIX),R=depthTIFF1:,PORT=FileTIFF1_D,NDARRAY_PORT=$(DEPTH_PORT)")

NDFileHDF5Configure("FileHDF1_D", $(QSIZE), 0, "$(DEPTH_PORT)", 0)
dbLoadRecords("$(ADCORE)/db/NDFileHDF5.template", "P=$(PREFIX),R=depthHDF1:,PORT=FileHDF1_D,NDARRAY_PORT=$(DEPTH_PORT)")
