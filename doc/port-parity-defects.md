...
## PP-101 [MED] Teledyne H `SetRefillRate` / `SetDigital` never leave the IOC — FIXED
- **Rust:** `iocs/syringepump-ioc/db/teledynePumpH.template:410,436` put the `@asyn` link on `calcout` records. EPICS asyn ships `device()` lines for ai/ao/bi/bo/mbbi/mbbo/mbboDirect/longin/longout/int64in/int64out/lsi/lso/stringin/stringout/waveform/aai/aao/printf and none for `calcout` (asyn-rs's vendored `dbd/devEpics.dbd`), so the DTYP resolves for no calcout in C, while asyn-rs's universal factory binds the record and simply never writes.
- **C:** upstream drives both through StreamDevice (`#field(DTYP, "stream")` remnants at `:401,403`); the asyn conversion is this port's own. The upstream SyringePump tree is not on this machine.
- **Impact:** the refill rate (the ml/min setpoint × 10) and the digital-output selector never reach the pump, and the records read back their computed value, so nothing on the CA side shows the command was dropped.
