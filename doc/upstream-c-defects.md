...
## Amptek DP5 (`drvAmptek.cpp` + bundled `AmptekSrc` DPP SDK)

| # | Defect | Port handling |
|---|--------|---------------|
| 216 | `AsciiCmdUtilities.h:17` — `#define Whitespace "\t\n\v\f\r\0x20"`: the intended space (`Chr$(32)`, per the adjacent comment) is written `\0x20`, which C parses as `\0` (NUL, terminating the string literal) followed by literal `x20`. The space is therefore never in the whitespace set, so internal spaces in a config line survive `RemoveWhitespace` and get sent to the DP5, whose `KEY=value;` grammar rejects them | fixed-in-port (`ascii_cmd.rs` `remove_whitespace` includes the space) |
| 217 | `DppSocket.cpp:428-450` — `CDppSocket::HaveNetFinderPacket`'s final `else` default-**accepts** (`return true`, logs `"no packet test"`) any buffer matching neither the valid NetFinder shape nor the "unknown packet type" shape, so arbitrary UDP broadcast traffic on the discovery socket is treated as a confirmed device response | fixed-in-port (`net_finder.rs` `have_netfinder_packet` default-rejects) |
| 218 | `ParsePacket.cpp:179-199` — `CParsePacket::ParseCmd` switches on `PIN->PID2` alone; PID2 is not globally unique (`RCVPT_CONFIG_READBACK` and `PID2_ACK_UNRECOG` are both `0x07`), so a valid config-readback reply to `XMTPT_SEND_CONFIG_PACKET_EX` misclassifies as "Unrecognized Command" | fixed-in-port structurally (`driver.rs` `send_command_string` keys off the joint `(pid1,pid2)` classification, making the collision unrepresentable) |
| 219 | `ParsePacket.cpp:23-26` — `ParsePacketStatus` reads the declared `LEN` bytes with no check that `raw` is at least that long, indexing past the buffer end on a short/truncated packet | fixed-in-port (`protocol.rs` `parse_packet_status` returns a length error instead of reading OOB) |

## epics-modules/measComp (`drvMultiFunction.cpp`, R4-4; parity audit 2026-09-22)

Confirmed statically against libuldaq 1.2.1 (`~/codes/libuldaq-1.2.1`) on the USB-2408-2AO path.

| # | Defect | Port handling |
|---|--------|---------------|
| 223 | `drvMultiFunction.cpp:2259` — `writeAnalogOutSync` calls `ulAOutArray` with `AOUTARRAY_FF_SIMULTANEOUS`, which the USB-24xx does not allow (`AoUsb24xx.cpp:18`), so `check_AOutArray_Args` returns `ERR_BAD_FLAG` (`AoDevice.cpp:307-308`) and the sync write never reaches the DACs | fixed-in-port (`NOSCALEDATA` only; libuldaq writes NO_UPDATE then UPDATE_ALL, `AoUsb24xx.cpp:96-104`, so both DACs still update together) |
| 224 | `drvMultiFunction.cpp:2154` — `ANALOG_OUT_SYNC_MASTER` → `ulAOSetConfig(AO_CFG_SYNC_MODE)` always returns `ERR_CONFIG_NOT_SUPPORTED` on the USB-24xx (base `AoDevice::setCfg_SyncMode`, `AoDevice.cpp:412-415`, not overridden) | not-reproduced (SyncMaster record omitted; template comment records why) |
| 225 | `drvMultiFunction.cpp:2604-2624` — the poller loops `ulDIn` over `numIOPorts_` = `DIO_INFO_NUM_PORTS` = 3 (`DioUsb24xx.cpp:15-17`); without an expansion board (only the USB-2416 sets `mHasExp`, `Usb24xx.cpp:71-76`) `ulDIn(AUXPORT1)` throws `ERR_BAD_PORT_TYPE` (`DioUsb24xx.cpp:41-42`) and the `goto error` skips the counter, AI and TC reads and the callbacks on every cycle | fixed-in-port (reads AUXPORT only) |
| 226 | `drvMultiFunction.cpp:225,1800,1989` — `startWaveDig` builds the queue with `aiInputMode_ == DIFFERENTIAL`, comparing the uldaq `AiInputMode` value (1/2) against the Linux `#define DIFFERENTIAL 0`, so the digitizer queue is always single-ended | fixed-in-port (queue uses the configured mode) |
| 227 | `drvMultiFunction.cpp:1653-1654` — the user-waveform amplitude/offset are read at the span index `i` instead of the channel `k` | fixed-in-port (the user waveform takes the absolute channel's Amplitude/Offset, `port-parity-defects.md` PP-94) |
| 228 | `measCompPulseGen_settings.req:2` — saves `Width`, a soft ao derived from `DutyCycle` by `CalcWidth`, and not `DutyCycle`, the driver record (`measCompPulseGen.template` DutyCycle PINI YES, VAL 0.5): at boot DutyCycle writes 0.5 and CalcWidth overwrites the restored Width, so every pulse generator comes back at 50 % duty. Confirmed live on the port: saved Width 2.43e-4 of Period 8.10e-4 → Width_RBV 4.05e-4 | fixed-in-port (`auto_settings.req` saves DutyCycle) |
| 229 | `drvUSBCTR.cpp:90,472` — `MAX_DELAY 67.11` is 2^32 counts of a 64 MHz clock; libuldaq runs the USB-CTR timer from 96 MHz (`UsbCtrx.cpp:21`) and rejects a delay over `UINT_MAX` counts (`TmrDevice.cpp:72-75`), so a delay in (44.74, 67.11] s is passed through and fails with `ERR_BAD_INITIAL_DELAY` instead of being clamped. Confirmed live: Delay 60 → "uldaq error 60: Invalid initial delay specified" | fixed-in-port (clamp at `u32::MAX / 96e6`) |
| 230 | `drvMultiFunction.cpp:1905-1915`, `:1738-1757` / `drvUSBCTR.cpp:873-880` — the digitizer, generator and MCS time axes are built from the requested dwell and never rebuilt from the rate `ulAInScan`/`ulAOutScan`/`ulDaqInScan` returns (drvUSBCTR even writes that dwell back to MCA_DWELL_TIME, `:711`, without recomputing), so after an adjusted-rate scan `TimeWF` disagrees with the samples. Live: WaveDigDwell 0.001 at Ai Rate 60 → DwellActual 0.01732, TimeWF step 0.001 | fixed-in-port (`fb63cb5`: each scan start republishes its axis from the dwell it runs at; a Dwell write still previews from the requested value. Live: digitizer step 0.001 → 0.01732 at start, generator 0.0123457 → 0.01234, MCS TimeWF republished with Dwell_RBV) |
| 231 | `drvMultiFunction.cpp:2178,2300` — `writeInt32`/`writeFloat64` call `defineWaveform(addr)` and drop its return, and `:1981,2020` drop `setOpenThermocoupleDetect`'s, so a point count over `maxOutputPoints_` and a failed `ulAISetConfig(OTD_MODE)` print their ERROR line and still return asynSuccess: the record that asked for the setting shows no alarm, and only the trace says it was refused. Live: `WaveGenIntNumPoints` 3000 of max 2048 → `defineWaveform: ERROR numPoints=3000 …` with a successful put | fixed-in-port (both failures fail the write, so the record alarms; the closing trace line is C's ERROR form where C prints its `wrote` line — `port-parity-defects.md` PP-100) |
