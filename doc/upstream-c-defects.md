...
| 230 | `drvMultiFunction.cpp:1905-1915` / `drvUSBCTR.cpp:873-880` — the digitizer and MCS time axes are built from the requested dwell and never rebuilt from the rate `ulAInScan`/`ulDaqInScan` returns, so after an adjusted-rate scan `TimeWF` disagrees with the samples. Live: WaveDigDwell 0.001 at Ai Rate 60 → DwellActual 0.01732, TimeWF step 0.001 | reproduced (port keeps C's requested-dwell axis; changing `TimeWF`'s meaning awaits sign-off) |
