...
| # | Defect | Port handling |
|---|--------|---------------|
| 220 | `drvMultiFunction.cpp:2259` — `writeAnalogOutSync` calls `ulAOutArray` with `AOUTARRAY_FF_SIMULTANEOUS`, which the USB-24xx does not allow (`AoUsb24xx.cpp:18`), so `check_AOutArray_Args` returns `ERR_BAD_FLAG` (`AoDevice.cpp:307-308`) and the sync write never reaches the DACs | fixed-in-port (`NOSCALEDATA` only; libuldaq writes NO_UPDATE then UPDATE_ALL, `AoUsb24xx.cpp:96-104`, so both DACs still update together) |
| 221 | `drvMultiFunction.cpp:2154` — `ANALOG_OUT_SYNC_MASTER` → `ulAOSetConfig(AO_CFG_SYNC_MODE)` always returns `ERR_CONFIG_NOT_SUPPORTED` on the USB-24xx (base `AoDevice::setCfg_SyncMode`, `AoDevice.cpp:412-415`, not overridden) | not-reproduced (SyncMaster record omitted; template comment records why) |
| 222 | `drvMultiFunction.cpp:2604-2624` — the poller loops `ulDIn` over `numIOPorts_` = `DIO_INFO_NUM_PORTS` = 3 (`DioUsb24xx.cpp:15-17`); without an expansion board (only the USB-2416 sets `mHasExp`, `Usb24xx.cpp:71-76`) `ulDIn(AUXPORT1)` throws `ERR_BAD_PORT_TYPE` (`DioUsb24xx.cpp:41-42`) and the `goto error` skips the counter, AI and TC reads and the callbacks on every cycle | fixed-in-port (reads AUXPORT only) |
| 223 | `drvMultiFunction.cpp:225,1800,1989` — `startWaveDig` builds the queue with `aiInputMode_ == DIFFERENTIAL`, comparing the uldaq `AiInputMode` value (1/2) against the Linux `#define DIFFERENTIAL 0`, so the digitizer queue is always single-ended | fixed-in-port (queue uses the configured mode) |
| 224 | `drvMultiFunction.cpp:1653-1654` — the user-waveform amplitude/offset are read at the span index `i` instead of the channel `k` | open in port — the port applies no user amplitude/offset at all (`port-parity-defects.md` PP-94); the fix there should index by absolute channel |
