...
//! helper is private), so the printf subset that `NDFileTemplate` — and the
//! marCCD series file templates — need is reimplemented here.
//!
//! `marCCD.cpp` builds three kinds of format string, all handled by
//! [`format_file_template`]:
//!  * `NDFileTemplate` (default `"%s%s_%3.3d.tif"`) for `createFileName`;
//!  * the user `SeriesFileTemplate` applied to `(filePath, fileName, fileNumber)`;
//!  * the run-time `fullFileTemplate` `"%s%<d>.<d>d.tif"` applied to
//!    `(baseFileName, frameNumber)`.
...
/// C `collectSeries`: `epicsSnprintf(fullFileTemplate, ..., "%%s%%%d.%dd.tif",`
/// `digits, digits)` — e.g. `digits == 5` yields `"%s%5.5d.tif"`.
pub fn full_file_template(digits: i32) -> String {
    format!("%s%{digits}.{digits}d.tif")
}
...
mod tests {
    ...
    #[test]
    fn template_default_marccd_form() {
        assert_eq!(
    ...
    #[test]
    fn full_file_template_builds_c_format() {
        assert_eq!(full_file_template(5), "%s%5.5d.tif");
        assert_eq!(full_file_template(4), "%s%4.4d.tif");
    }

    #[test]
    fn full_file_template_applied() {
        // C: baseFileName + %5.5d + ".tif" for frame i+first.
        let tmpl = full_file_template(5);
        assert_eq!(
            format_file_template(&tmpl, "/data/base", "", 1).unwrap(),
            "/data/base00001.tif"
        );
        assert_eq!(
            format_file_template(&tmpl, "/data/base", "", 1234).unwrap(),
            "/data/base01234.tif"
        );
    }
