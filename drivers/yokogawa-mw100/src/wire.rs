//! Raw TCP framing (`drvMW100_comm.c`): every response starts with a 2-byte
//! `E`+type header (`mw100_response_reader`). `'0'`=OK, `'1'`=ERROR,
//! `'2'`=CHAIN_ERRORS, `'A'`=ASCII (terminated by `"EN\r\n"`), `'B'`=BINARY
//! (8-byte header, 4-byte length at offset 4, then that many more bytes).
//! Every write is preceded by a 20ms sleep (`mw100_simple_writer`/
//! `mw100_writer`, `drvMW100_comm.c:111-121`).
//!
//! Unlike GM10, the binary length field is native/host byte order, not
//! `ntohl`'d (`mw100_binary_reader`, `drvMW100_comm.c:97-98`:
//! `dp = (uint32_t *)(comm->inbuffer + 4); datalen = *dp;` — no byte-swap).
//! `init_mw100` negotiates this explicitly: it sends `"BO1\r\n"` only under
//! `#if EPICS_BYTE_ORDER == EPICS_ENDIAN_LITTLE` (`drvMW100.c:1538-1543`),
//! else `"BO0\r\n"`. This port always sends `BO1` and decodes little-endian,
//! matching the only byte order any real little-endian host (the only kind
//! this crate targets) would negotiate.
...
/// (`mw100_binary_reader`: `datalen = *(uint32_t*)(inbuffer+4)`, native byte
/// order, total = `8 + datalen`.)
pub fn binary_frame_total_len(header: &[u8; 8]) -> Option<usize> {
    if &header[0..2] != b"EB" {
        return None;
    }
    let datalen = u32::from_le_bytes([header[4], header[5], header[6], header[7]]);
...
pub enum RawResponse {
    ...
    Error(Vec<u8>),
    /// `"E2,...\r\n"` (chained-error variant, not decoded further).
    ChainErrors(Vec<u8>),
...
/// Read one full response frame (`mw100_response_reader`).
...
/// (`mw100_simple_writer`/`mw100_writer`).
...
/// `drvMW100.c` consumer skips via `ptr += 4` without inspecting it, and the
...
mod tests {
    ...
    #[test]
    fn binary_header_total_len_is_little_endian() {
    ...
    fn read_response_dispatches_ascii_and_strips_payload() {
        let mut cursor = io::Cursor::new(b"EAxxIS0 payload\r\nEN\r\n".to_vec());
        match read_response(&mut cursor).unwrap() {
            RawResponse::Ascii(raw) => {
                assert_eq!(ascii_payload(&raw), b"IS0 payload\r\n");
            }
